...
export function ShortenStylePrimaryLink({ to, children, className = "", showArrow = true, size = "default" }) {
  ...
  return (
    ...
    >
      <Link
        to={to}
        className={`group/btn relative overflow-hidden cursor-pointer bg-blue-500/15 dark:bg-blue-600/20 backdrop-blur-xl border border-blue-500/30 dark:border-blue-400/40 border-t-blue-400/20 dark:border-t-white/20 text-blue-600 dark:text-white font-display font-black rounded-2xl sm:rounded-full transition-all flex items-center justify-center gap-2 active:scale-[0.97] ${linkSize}`}
      >
